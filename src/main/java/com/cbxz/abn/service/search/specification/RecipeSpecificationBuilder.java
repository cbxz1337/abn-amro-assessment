...
public class RecipeSpecificationBuilder {
    ...
    public Specification<Recipe> build() {
        ...
        RecipeSpecification specification = new RecipeSpecification(criteriaList.getFirst());
        for (int i = 1; i < criteriaList.size(); i++) {
            val criteria = criteriaList.get(i);
            switch (criteria.joinType()) {
                case OR -> Specification.where(specification).or(new RecipeSpecification(criteriaList.get(i)));
                case AND -> Specification.where(specification).and(new RecipeSpecification(criteriaList.get(i)));
            }
        }
